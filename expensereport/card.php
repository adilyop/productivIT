...
if (empty($reshook)) {
	...
	if ($action == 'add' && $user->rights->expensereport->creer) {
		...
		if (
			(empty($conf->global->MAIN_USE_ADVANCED_PERMS) && empty($user->rights->expensereport->creer))
...
if ($action == 'create') {
	...
	if ($result > 0) {
		if (!in_array($object->fk_user_author, $user->getAllChildIds(1))) {
			if (
				empty($user->rights->expensereport->readall) && empty($user->rights->expensereport->lire_tous)
